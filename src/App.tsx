...
function AppInner() {
  ...
  // Auth initialization: restore from localStorage first, then sync with Firebase
  useEffect(() => {
    ...
    // Then verify/sync with Firebase
    const unsubscribe = auth.onAuthStateChanged(async (firebaseUser) => {
      try {
        if (firebaseUser) {
          ...
          localStorage.setItem("panadev_token", token);
          setIsSignInModalOpen(false);
        } else {
          // Only clear if there's no custom token (admin login)
          const customToken = localStorage.getItem("panadev_token");
          if (!customToken) {
            setIdToken(null);
            setUser(null);
          }
  ...
  // Token refresh interval - refresh every 55 minutes (tokens expire after 60)
  useEffect(() => {
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setIsSignInModalOpen(false)}>
          <div className={`w-full max-w-sm rounded-2xl border ${theme.card} p-8 space-y-6 text-center`} onClick={e => e.stopPropagation()}>
            ...
            <div className="relative flex items-center justify-center py-1">
              ...
              <span className="relative px-3 text-[10px] text-slate-500 uppercase tracking-widest bg-slate-950 font-bold">Or</span>
            </div>
            ...
            <button
              ...
              disabled={authLoading}
              type="button"
              className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-slate-900 hover:bg-slate-850 text-slate-300 font-bold text-xs border border-slate-800 shadow-sm transition cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {authLoading ? (
